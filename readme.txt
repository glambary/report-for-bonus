...
При использовании, прошу следовать следующим шагам:
	1) скачать или склонировать проект;
	2) создать виртуальное окружение;
	3) воспользоваться командой pip freeze > requirements.txt

К сожалению, файлы zap.xlsx, data.xlsx загрузить не могу, так как в них содержится коммерческая тайна.

-------------------------------------------------------------------------------------------

Script (program) to calculate bonuses for sales staff.
Used libraries: OpenPyXl, PyInstaller (to convert to "exe" format).
Working principle:
	1) reading the file zap.xlsx and selecting rows suitable by the condition set by the customer;
	2) Reading the data.xlsx file and selecting rows suitable for the condition set by the customer;
